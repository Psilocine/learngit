...
### 2019年11月11日
1. vue-cli2的本地跨域代理 2019年10月22日 讨论过。vue-cli3下该如何操作呢？它并没有暴露webpack的配置文件，此时我们可以在vue.config.js中配置 devServer 的 proxy

### 2019年11月1日
1. 今天排查出了困扰许久多问题，明明在自己的目录下，npm竟然permission denied。网上查阅了许多都不能改变，给文件夹赋权限等。后面发现是自己蠢了，在/usr/local/lib里的全局依赖全都属于一个‘777’的用户。原来是赋777权限的时候命令错了导致把全局依赖的所属用户给了‘777’这个用户，而且命令错了，只有‘777’有rwx的权限，其他用户没有w的权限导致了这场闹剧。（我太难了
